...
class Tracking extends Base
{
    ...
    { 
        ...
        if($listId > 0 && $clientId > 0)
        {
            ...
            $dataList = $this->app->database('system')->query()->from('lists.data_lists',['table_schema','table_name'])->where('id = ?',$listId)->first();
        ...
        if($processId > 0 || $offerId > 0)
        {
            if($processId > 0)
            {
                ...
                $process = $this->app->database('system')->query()->from("production.{$table}",['id','offer_id','isp_id','user_id'])->where('id = ?',$processId)->first();
            ...
            $offer = $this->app->database('system')->query()->from('affiliate.offers',['id','affiliate_network_id'])->where('id = ?',$offerId)->first();
            ...
            $creative = $this->app->database('system')->query()->from('affiliate.creatives',['id'])->where('offer_id = ?',$offerId)->order('id')->first();
            ...
            $link = $this->app->database('system')->query()->from('affiliate.links',['type','value'])->where('creative_id = ? AND type = ?',[intval($creative['id']),$type])->first();
            ...
            if($type == 'preview')
            {
                ...
                $affiliateNetwork = $this->app->database('system')->query()->from('affiliate.affiliate_networks',['id','api_type','sub_id_one','sub_id_two','sub_id_three'])->where('id = ?',intval($offer['affiliate_network_id']))->first();
    ...
    { 
        ...
        if($processId > 0 || $offerId > 0)
        {
            if($processId > 0)
            {
                ...
                $table  = $processType == 'mt' || $processType == 'md' ? 'mta_processes' :  'smtp_processes';
                $process = $this->app->database('system')->query()->from("production.{$table}",['id','offer_id','isp_id','user_id','auto_responders_ids'])->where('id = ?',$processId)->first();
            ...
            # get offer
            $offer = $this->app->database('system')->query()->from('affiliate.offers',['id','production_id','affiliate_network_id'])->where('id = ?',$offerId)->first();
            ...
            # get mailer 
            $user = $this->app->database('system')->query()->from('admin.users',['first_name','last_name'])->where('production_id = ?',$userId)->first();
            ...
            if($actionObject != null)
            {
                ...
                {
                    $ids = [];
                    $autoResponders = $this->app->database('system')->query()->from('production.auto_responders',['id'])->where("id IN ? AND status = ? AND $autoColumn = 't'",[$autoRespondersIds,'Activated'])->all();
            ...
            if($listId > 0 && $clientId > 0)
            {
                ...
                $list = $this->app->database('system')->query()->from('lists.data_lists',['id','name','table_schema','table_name','isp_id'])->where('id = ?',$listId)->first();
    ...
    { 
        ...
        {
            $list = $this->app->database('system')->query()->from('lists.data_lists',['table_schema','table_name'])->where('id = ?',$listId)->first();
